...
// 🔹 Ambil data periode observasi APD
$q = mysqli_query($conn, "SELECT * FROM data_observasi WHERE id='$id_observasi' LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $fields_query = mysqli_query($conn, "SELECT field_name FROM apd_fields ORDER BY id ASC");
    $fields = [];
    ...
    // 🔹 Query insert baru dengan field tindakan
    $sql = "INSERT INTO observasi_apd 
        (id_observasi, bulan, ruangan, tanggal, petugas, nama_rekan_dilaporkan, tindakan,
         $field_names, numerator, denumerator)
    VALUES (
        '$id_observasi',
        '{$data_observer['bulan']}',
...
<title>Form Observasi - <?= htmlspecialchars($data_observer['ruangan']) ?></title>
...
/* 🔸 Warna berbeda untuk select pilihan */
select.form-select {
...
select[value="Tidak"] {
    background-color: #ffcdd2 !important; /* merah lembut */
}
select[value="Tidak Dinilai"] {
    background-color: #f0f0f0 !important; /* abu lembut */
}

input[readonly] {
  background-color: #e9ecef !important; /* abu-abu khas input terkunci Bootstrap */
  color: #555;
  cursor: default !important; /* tidak muncul pointer */
...
<div class="logout-box d-flex align-items-center gap-2">
...
<div class="form-container">
    <div class="text-center mb-3">
        ...
        <h5>RSUP Jayapura</h5>
        <p style="color:#388e3c;">Form Observasi APD System</p>
    </div>

    <a href="form_input.php" class="btn btn-secondary w-100 mb-3">← Kembali ke Daftar Ruangan</a>

    <h3>🧾 Observasi: <?= htmlspecialchars($data_observer['ruangan']) ?></h3>

<?php
function formatBulanIndonesia($tanggal_bulan) {
    $bulan = [
        '01'=>'Januari','02'=>'Februari','03'=>'Maret','04'=>'April','05'=>'Mei','06'=>'Juni',
        '07'=>'Juli','08'=>'Agustus','09'=>'September','10'=>'Oktober','11'=>'November','12'=>'Desember'
    ];
    $parts = explode('-', $tanggal_bulan);
    if (count($parts) == 2) {
...
<p>
   <strong>Bulan:</strong> <?= formatBulanIndonesia($data_observer['bulan']) ?><br>
...
<div class="popup-success" id="popupSuccess">
    ...
    <p>Data observasi telah tersimpan dan diteruskan ke admin.</p>
</div>
<script>
setTimeout(() => {

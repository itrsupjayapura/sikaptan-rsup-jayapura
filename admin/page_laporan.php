...
if ($show_data) {
    $sql = "SELECT oa.id_observasi, do.bulan, do.ruangan, oa.tanggal, oa.petugas, 
        oa.nama_rekan_dilaporkan, oa.tindakan, oa.numerator, oa.denumerator";
...
<form method="GET" class="filter-form">
    ...
    <div class="filter-buttons">
        ...
        <?php if ($filter_bulan || $filter_ruangan): ?>
            <a href="?page=laporan" class="btn-reset">♻️ Reset</a>
        <?php endif; ?>
    </div>
</form>
...
<!-- ===== EXPORT ===== -->
<?php if ($show_data): ?>
    <form method="POST" action="page_laporan_export.php" target="_blank" class="export-form">
...
if (!$show_data): ?>
    ...
    foreach ($rows as $row):
        ...
        // 🧩 Tambahan agar "Nama Rekan Dilaporkan" dihitung
        $nama_rekan = trim($row['nama_rekan_dilaporkan']);
        if ($nama_rekan !== '') {
            $nama_dilaporkan[$nama_rekan] = true; // gunakan nama sebagai key agar tidak dobel
        }
                    ...
                    foreach ($rows as $row): ?>
                        <tr>
                            ...
                            <?php foreach ($fields as $key => $label): ?>
                                <?php
                                $val = trim(strtolower($row[$key] ?? ''));
                                $bg_color = ''; // default tanpa warna

                                if ($val == 'ya') {
                                    $bg_color = 'background-color:#d4edda; color:#155724;'; // hijau muda
                                } elseif ($val == 'tidak') {
                                    $bg_color = 'background-color:#f8d7da; color:#721c24;'; // merah muda
                                } elseif ($val == 'tidak dinilai') {
                                    $bg_color = 'background-color:#fff3cd; color:#856404;'; // kuning muda
                                }
                                ?>
                                <td style="<?= $bg_color ?>"><?= htmlspecialchars($row[$key]) ?></td>
                            <?php endforeach; ?>
            ...
            new Chart(document.getElementById('chartPie'), {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Distribusi Jawaban (Tanpa Tidak Dinilai)',
                            font: {
                                size: 16,
                                weight: 'bold'
                            }
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(ctx) {
                                    ...
                                    const persen = ((val / totalValid) * 100).toFixed(1);
                        ...
                        datalabels: {
                            ...
                            formatter: (val) => {
                                return ((val / totalValid) * 100).toFixed(1) + '%';
                            }
                        }
                    }
                },
            ...
            new Chart(ctxBar, {
                ...
                data: {
                    ...
                    datasets: [{
                            ...
                            backgroundColor: warnaYa
                        },
                        {
                            label: 'Tidak',
                            data: labels.map(k => statData[k]['Tidak']),
                            backgroundColor: warnaTidak
                        }
                    ]
